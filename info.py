...
BOT_TOKEN = environ.get('BOT_TOKEN', '8098647837:AAHI4a5siXqzhwSBU2O4kWrvXsOZqbuIkbw')
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001999941677 -1002591922002').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_grp = environ.get('AUTH_GROUP')
DEFAULT_AUTH_CHANNELS = [int(x) for x in environ.get("AUTH_CHANNEL", "-1003602676231 -1003300491679").split() if x.lstrip('-').isdigit()]
